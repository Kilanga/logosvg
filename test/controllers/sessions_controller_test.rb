...
class SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "the right credentials open a session" do
    post session_path, params: { email_address: users(:client).email_address, password: "motdepasse-test" }

    assert_redirected_to "/mon-espace"
  end

  test "the address is matched whatever the case" do
    post session_path, params: { email_address: users(:client).email_address.upcase, password: "motdepasse-test" }

    assert_redirected_to "/mon-espace"
  end

  test "a wrong password says the same thing as an unknown address" do
    post session_path, params: { email_address: users(:client).email_address, password: "pas-le-bon" }
    wrong_password = flash[:alert]

    post session_path, params: { email_address: "inconnue@example.invalid", password: "motdepasse-test" }
  ...
  test "a deleted account cannot sign back in" do
    post session_path, params: { email_address: users(:deleted_client).email_address, password: "motdepasse-test" }
